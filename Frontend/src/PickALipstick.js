...
import HeaderLG from 'VirtualLip/src/components/headerGoback.js';
import { Auth } from 'aws-amplify';
handleReplaceColor = ()=>{
  alert('功能尚在開發中...')
}
function PickALipstick({ route, navigation }) {
  let {tarLipstick} = route.params;
  let {tarColor} = route.params;
  let {photo} = route.params;
  console.log(tarLipstick);
  console.log(tarColor);
  console.log(photo);
  return (
    <Container>
      <HeaderLG navigation={navigation} />
      ...
      <Container style={{ flex: 1, marginLeft: 40, marginRight: 40 }}>
        <Button
          ...
          rounded
          light
          style={{ backgroundColor: tarColor, marginBottom: 20 }}
          onPress={this.handleReplaceColor}
        >
          <Text style={{ color: 'white' }}>試用 {tarLipstick}{tarColor}</Text>
        </Button>
        <TouchableHighlight
...
//   }
//   componentDidMount(){
//     this.setState({name: this.props.navigation.getParam('tarcolor')})
